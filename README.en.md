...
- **Scheduled restores** *(new)*: recurring **restore tests** (restore a VM's latest
...
- **Users and roles**: administrators (manage everything) and operators; the admin can
  create/edit users, change passwords and **reset other users' 2FA**.
...
Configuration lives in `/etc/pbi/pbi.env` and persistent data (hosts, users, jobs, etc.)
in `/var/lib/pbi`.
...
└─ web/                     # React + Vite frontend
   └─ src/
      ├─ App.jsx            # navigation + host selector
