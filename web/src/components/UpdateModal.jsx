...
/** Comprueba la última release de GitHub y ofrece descargar el .deb. */
      ...
      <div className="modal" style={{ maxWidth: 560 }} onClick={(e) => e.stopPropagation()}>
        ...
        {state.loading ? (
          ...
          <>
            ...
            {state.notes && (
              <div style={{ maxHeight: 180, overflow: 'auto', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', margin: '4px 0 12px', fontSize: 12.5, whiteSpace: 'pre-wrap', color: 'var(--text-2)' }}>
                {state.notes.slice(0, 1200)}{state.notes.length > 1200 ? '…' : ''}
              </div>
            )}
            <div className="btn-row" style={{ marginBottom: 12 }}>
              {state.debUrl && <a className="btn primary" href={state.debUrl}><Icon.download width={14} height={14} /> {t('Descargar .deb')}</a>}
              <a className="btn" href={state.url} target="_blank" rel="noreferrer">{t('Ver release')}</a>
            </div>
            {cmd && (
              <div className="field">
                <label>{t('Instala en el servidor PBI (requiere root):')}</label>
